...
public class OrExpressionTest {
    ...
    public void testHalfIntersection(){
        ...
        OrExpression orExpression = new OrExpression(is42, isNotKenny);

        Assertions.assertEquals(3, is42.evaluate().size());
        Assertions.assertEquals(3, isNotKenny.evaluate().size());
        var result = orExpression.evaluate();
        Assertions.assertEquals(4, result.size());
        Assertions.assertTrue(isAllPresented(Arrays.asList(1L,2L, 4L, 6L),result));
